...
class MessageShow : public Componentality::Graphics::JetCat::ActiveArea
{
protected:
	std::list<Componentality::Graphics::Point> mPoints;
	Componentality::Graphics::JetCat::StdEventFilter filter;
public:
	MessageShow(IElement* owner) { setEventFilter(&filter); };
	virtual ~MessageShow() {};
	Componentality::Graphics::JetCat::Margins draw(Componentality::Graphics::ISurface& surface)
	{
		Componentality::Graphics::Drawer drawer(surface);
		for (std::list<Componentality::Graphics::Point>::iterator i = mPoints.begin(); i != mPoints.end(); i++)
		{
			Componentality::Graphics::Point point = *i;
			Componentality::Graphics::Point shift = getTopLeft();
			point = point - shift;
			drawer.point(point, Componentality::Graphics::Color(0xFF, 0xFF, 0xFF));
		}
		return IElement::draw(surface);
	}
	bool onEvent(CST::Common::Event& event)
	{
		/*		if (event.getType() == Componentality::Graphics::JetCat::EVENT_TYPE_POINTER)
		{
		Componentality::Graphics::JetCat::PointerEvent& _event = (Componentality::Graphics::JetCat::PointerEvent&) event;
		mPoints.push_back(_event.get());
		raise(*new Componentality::Graphics::JetCat::DrawRequestEvent(this));
		}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		Componentality::Graphics::JetCat::Window& painter = *new Componentality::Graphics::JetCat::Window(*Componentality::Graphics::Windows::DEFAULT_WINDOW_MANAGER);
		...
		painter.setEventFilter(new Componentality::Graphics::JetCat::StdEventFilter());
